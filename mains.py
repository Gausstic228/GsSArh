...
def return_phones():
    ...
    for i in range(int(len(fav_phones)/2)):
        print(f"{i} - {fav_phones[i*2]} {fav_phones[i*2+1]}")
...
def save_phones():
    ...
    while True:
        ...
        if _menu == "2":
            delete_index = int(input("Введите номер, который хотите удалить: "))
            fav_phones.pop(delete_index * 2)
            fav_phones.pop(delete_index * 2)
        with open('phones.data', 'w') as filehandle:
            for listitem in fav_phones:
                filehandle.write('%s\n' % listitem)
...
def settings():
    ...
    while True:
        console.clear()
        console.print(Panel(Text(banner, style="bold green"), title=f"Настройки - Версия {version}", style="cyan"))
        console.print(f"1 - Выходить из программы поле бомбинга: [bold yellow]{set[0]}[/bold yellow]")
        console.print(f"2 - Вероятность бомбинга звонками: [bold yellow]{set[1]}%[/bold yellow]")
        console.print("\n[bold red]0 - Выход из этого меню и сохранение[/bold red]")
        menu = console.input("\n[bold green]Ваш выбор: [/bold green]")
        if menu == "0":
            with open('config.data', 'w') as filehandle:  
                for listitem in set:
                    filehandle.write('%s\n' % listitem)
            ...
            set[1] = float(console.input("\nВведите шанс звонка: "))

def info():
    global banner, version
    console.clear()
    console.print(Panel(Text(banner, style="bold green"), title="Информация", style="magenta"))
    console.print(f">> Версия: [bold yellow]{version}[/bold yellow]")
    console.print(">> Создатель: [bold yellow]GsS[/bold yellow]")
    console.print("\n[bold red]Нажмите ENTER чтобы выйти[/bold red]")
    input()

while True:
    console.clear()
    console.print(Panel(Text(banner, style="bold green"), title=f"Bomber {version}", style="green"))
